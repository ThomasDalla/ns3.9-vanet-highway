...
static void Start(Ptr<Highway> highway)
{
  highway->Start();
    // test
  //Ptr<Vehicle> v1=highway->FindVehicle(1);
  //Ptr<Vehicle> v1=CreateObject<Vehicle>();
  //v1=highway->FindVehicle(1);
  //v1->SetVelocity(60);
  //v1->SetLane(1);
  //v1->SetVehicleId(1);
  //cout << v1->GetVelocity() << endl;
  //highway->AddVehicle(v1);
}
...
int main (int argc, char *argv[])
{
  ...
  cmd.AddValue ("std2", "traffic flow std at entrance (other direction)", std2);
  ...
  //vehicle1->setLaneChange(true)
  //Vector v(0,0)
  //vehicle1->setPosition(v)
